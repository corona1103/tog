...
## 2026-09-15  启用知识点正确率 MCP

- 改动人：PM + Codex 协作
- 改动：新增 `queryStudentKnowledgeAccuracy.yaml`，并在单文件系统 Prompt 中启用正确率查询和 0～100 百分比解读规则。
- 原因：接口已恢复可用，并提供了包含作答次数、答对次数、正确率和题目数的真实返回示例。
- 预期影响 case：知识点正确率查询、薄弱知识点推荐和学情复盘 case。
- 评估对比：需补充正确率单位、空知识点名称和小样本场景回归。
- 评审：待 PM、研发和教研 review
